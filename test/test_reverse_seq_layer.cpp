...
namespace caffe{
	...
	class ReverseSeqLayerTest{
	public:
		ReverseSeqLayerTest() : x_(new Blob<Dtype>()),cont_(new Blob<Dtype>()), x_rev_(new Blob<Dtype>()){
			this->SetUp();
		}

		void TestSetUp(){
			shared_ptr<Layer<Dtype>> layer(new ReverseSeqLayer<Dtype>(layer_param_));
		...
		void TestGradients(Caffe::Brew mode){
			...
			ReverseSeqLayer<Dtype> layer(layer_param_);
...
int main(int argc, char** argv){
	...
	caffe::ReverseSeqLayerTest<float> test;
	test.TestSetUp();
