...
namespace caffe{
		...
		for (int i = 0; i < n; ++i){
			...
			for (int j = 0; j < n - (i + 1); ++j){
				if (values[j] < values[j + 1]){
					swap<Dtype>(values, i, j);
					swap<Dtype>(indexes, i, j);
					swapped = true;
				}
			}
			if (!swapped){ break; }
  ...
  /*
   * nthreads = num * channels * outer_num * outer_num
   */
	template<typename Dtype>
	__global__ void ComputeBlockOffset(const int nthreads, const int split_num, 
		const int pyramid_height,
		const int outer_dim, const int outer_num, const int num_unit_block,
		...
		CUDA_KERNEL_LOOP(index, nthreads){
			...
			int ow = nthreads % outer_num;
			int oh = nthreads / outer_num;
			int c = nthreads / outer_dim / outer_dim;
			int inner_dim = outer_dim / split_num;
			int ooffset = c * num_unit_block * num_unit_block + oh * outer_dim * num_unit_block
				+ ow * outer_dim;
			for (int ih = 0; ih < split_num; ++ih){
				for (int iw = 0; iw < split_num; ++iw){
					...
					indexes[ih * split_num + iw] = ih * split_num + iw;
				}
			}
			//sort
			...
			for (int b = 0; b < num_part; ++b){
				...
				int offset_h = (sorted_bh - source_bh) * inner_dim * 
					num_unit_block * unit_block_height;
	...
	void PatchRankLayer<Dtype>::GetBlockOffset_gpu(){
		...
		for (int p = 0; p < pyramid_height_; ++p){
			...
			ComputeBlockOffset<Dtype> << <CAFFE_GET_BLOCKS(nthreads), CAFFE_CUDA_NUM_THREADS >> >(
				nthreads, split_num_, pyramid_height_, outer_dim, outer_num, num_unit_block_,
	...
	void PatchRankLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		PatchRankForward<Dtype> << < CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >(
			count, unit_block_height_, unit_block_width_, channels_, height, 
	...
	void PatchRankLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
		...
		PatchRankBackward<Dtype> << < CAFFE_GET_BLOCKS(count), CAFFE_CUDA_NUM_THREADS >> >(
			count, unit_block_height_, unit_block_width_, channels_, 
