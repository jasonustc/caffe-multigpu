...
void TripletLossLayer<Dtype>::Forward_gpu(
  ...
  for (int i = 0; i < bottom[0]->num();++i) {
     mdist=sampleW[i]* std::max(margin+dist_sq_ap_.cpu_data()[i]- dist_sq_an_.cpu_data()[i], Dtype(0.0)); //
     loss +=mdist;
     //LOG(INFO)<<"fap is equal to: "<<dist_sq_ap_.cpu_data()[i]<<"    fan is equal to: "<<dist_sq_an_.cpu_data()[i];
     //LOG(INFO)<<"margin mdist is equal to:  "<<margin +dist_sq_ap_.cpu_data()[i] - dist_sq_an_.cpu_data()[i];
     //LOG(INFO)<<"mdist is equal to: "<< mdist;
     //LOG(INFO)<<"loss is equal to : "<< loss;
  }
