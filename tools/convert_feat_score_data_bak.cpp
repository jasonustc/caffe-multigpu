...
void parse_line_feat(string& line, vector<float>& feat){
	...
	boost::trim(line);
	boost::split(strs, line, boost::is_any_of(FLAGS_spliter));
	float feat_i;
...
void load_file_list(const string& file, vector<string>& file_list){
...
void convert_dataset_float (const string& list_file, const string& db_name){
	...
	for (size_t f = 0; f < file_list.size(); f++){
		parse_file_feat(file_list[f], feats);
		//check feat dim 
...
int main(int argc, char** argv) {
	...
	if (argc < 3) {
		gflags::SetUsageMessage("Convert feats and scores by line to lmdb/leveldb\n"
			...
			"Usage: \n"
			"EXE [FLAGS] INPUT_FEAT_LIST_FILE DB_NAME\n");
