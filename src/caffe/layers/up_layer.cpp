  ...
  for (int i = 0; i < NUM_T_; ++i) {
    if (this->layer_param_.transformations(i).scale() == 1 &&
      ...
      if (!this->layer_param_.transformations(i).has_final_width()) {
        ...
        coord_idx_[i].reset(new Blob<float>()); 
        ...
        case NN:
          coord_idx_[i].reset(
              new Blob<float>(1, 1, height_new * width_new, 1));
          break;
        case BILINEAR:
          coord_idx_[i].reset(
              new Blob<float>(1, 1, height_new * width_new * 4, 1));
          break;
        default:
...
INSTANTIATE_CLASS_FLOAT_ONLY(UpsamplingLayer);

REGISTER_LAYER_CLASS_FLOAT_ONLY(Upsampling);
